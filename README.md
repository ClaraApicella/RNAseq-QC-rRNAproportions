...
### Gene Annotation
An additional utility `annotate_matrices.py` is included to query Ensembl BioMart for gene annotations (Symbol, Description, Biotype) and append them as columns directly into the original count matrix. 

```bash
python annotate_matrices.py config_template.yaml
```
Using the exact same input configuration file, this will output an `annotated_<original_basename>.xlsx` file containing the count data with explicit gene-level annotations attached immediately following the `Geneid` column.
